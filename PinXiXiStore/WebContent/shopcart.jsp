    ...
    <div class="cart w1200">
      ...
      <div class="OrderList">
        <div class="order-content" id="list-cont">
        	<c:forEach var="goods" items="${cartAll.data}" varStatus="s">
        	<input type="hidden" class="gid" value="${goods.gid}">
            ...
            <li class="th th-item">
              <div class="item-cont">
                <a href="javascript:;"><img src="/userphoto/${goods.goodsimage}" alt=""></a>
                <div class="text">
                  <div class="title">${goods.goodsname}</div>
                  <p><span>${goods.reserved1}</span></p>
                </div>
              </div>
            </li>
            <li class="th th-price">
              <span class="th-su">${goods.goodsprice}</span>
            </li>
            <li class="th th-amount">
              <div class="box-btn layui-clear">
                <div class="less layui-btn">-</div>
                <input class="Quantity-input gtotal" type="" name="" value="${goods.total}" disabled="disabled">
                <div class="add layui-btn">+</div>
              </div>
            </li>
            <li class="th th-sum">
              <span class="sum">${goods.sumprice}</span>
            </li>
            <li class="th th-op">
              <span class="dele-btn" onclick="del(this)">删除</span>
            </li>
          </ul>
        	</c:forEach>
<!--         	<script type="text/javascript"> -->
...
<!--         	</script> -->
      ...
      <div class="FloatBarHolder layui-clear">
        ...
        <div class="th Settlement">
          <button class="layui-btn" onclick="subproduct()">结算</button>
          <script type="text/javascript">
          	function subproduct(){
          		//获取用户id
          		var uid=$('#uid').val();
          		...
          		//截取最后一位
          		gids=gids.substring(0,gids.length-1);//商品id串
          		//获取每一个商品的个数
          		var $goodscount=$('.gtotal');
          		//console.info($goodscount);
          		var goodscount="";
          		...
          		//console.info(goodscount);//商品数量字符串
          		//获取应付款金额
          		var pay_money=$('#deu_money').text();
          		//截取出金额
          		pay_money=pay_money.substring(1,pay_money.length)
          		//console.info(pay_money);

          		//向后台发请求
          		location.href="order?method=paypage&uid="+uid+"&goodscount="+goodscount+"&gids="+gids+"&money="+pay_money;
        ...
        <div class="th total">
          <p>应付：<span class="pieces-total" id="deu_money">0</span></p>
        </div>
      </div>
    </div>
  </div>
