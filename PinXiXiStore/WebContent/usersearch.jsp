...
<link href="res/static/one/css/seastyle.css" rel="stylesheet" type="text/css" />
...
//   	})
      ...
      <div class="headerCon">
        ...
        <div class="mallSearch">
          ...
          <input type="hidden" name="method" value="goodsbytitle">
            <input type="text" name="title" required  lay-verify="required" autocomplete="off" style="hei" class="layui-input" placeholder="请输入需要的商品">
  ...
  <div class="content content-nav-base information-content" >
    ...
    <div class="info-list-box" style="padding:30px 0 84px;">
						...
						<div class="am-u-sm-12 am-u-md-12">
							<div class="search-content">
								...
								<script type="text/javascript">
									...
									function salesSort(){//按销量排序
										//$('#sales').parent().addClass('first');
										//$('#price').parent().removeClass('first');
										//获取地址栏的搜索内容
										//var title=location.href.split("title=")[1];
										var title=getUrlSearch('title');
										//alert(title);
										if(title!="no"){
											//alert('输入了');
											//如果用户搜索了商品则排序模糊查询出来的商品(按销量)
											location.href="goods?method=salestoinput&title="+title;
										}else{
											//alert('没输入');
											//如果用户没有输入具体的商品则排序所有商品(按销量)
											location.href="goods?method=sales";											
										}
										
										
									}
									function priceSort(){//按价格排序
										//$('#sales').parent().removeClass('first');45
										//$('#price').parent().addClass('first');
										//获取地址栏的搜索内容
										var title=getUrlSearch('title');
										if(title!="no"){
											//如果用户搜索了商品则排序模糊查询出来的商品(按价格)
											location.href="goods?method=priceinput&title="+title;
										}else{
											//如果用户没有输入具体的商品则排序所有商品(按价格)
											location.href="goods?method=price"
										}
										
									}
									
									$(function(){
									var dr=location.href.split("dr=")[1];
										if(dr=='sales'){//按销量排序
											//console.info($('#sales').parent());
											$('#sales').parent().addClass('first');
											$('#price').parent().removeClass('first');
										}
										if(dr=='price'){//按价格排序
											//console.info($('#price').parent());
											$('#sales').parent().removeClass('first');
											$('#price').parent().addClass('first');
										}
								...
								<ul>
									<li class=""><a id="sales" title="销量" href="javascript:void(0)" onclick="salesSort()">销量排序</a></li>
									<li class=""><a id="price" title="价格" href="javascript:void(0)" onclick="priceSort()">价格优先</a></li>
								</ul>
								...
								<script type="text/javascript">
									function choose(t){
										var gid=$($(t).children()[0]).children()[0].value;
										//向后台发请求  显示该商品的详细信息
								...
								<ul class="am-avg-sm-2 am-avg-md-3 am-avg-lg-4 boxes">
									<c:forEach var="goods" items="${dimgoodslist}" varStatus="s">
										<li onclick="choose(this)">
											<div class="i-pic limit ">
											<input type="hidden" id="hiddengid" value="${goods.gid}"><!-- 商品id -->
											<input type="hidden" id="hiddengoodsnumber" value="${goods.goodsnumber}"><!-- 商品编号 -->
											<input type="hidden" id="hiddengoodstotal" value="${goods.goodstotal}"><!-- 商品库存 -->
											<input type="hidden" id="hiddenmid" value="${goods.mid}"><!-- 商家id -->
											<input type="hidden" id="hiddengoodsinfo" value="${goods.goodsinfo}"><!-- 商品简介 -->
											<input type="hidden" id="hiddendiscountprice" value="${goods.discountprice}"><!-- 商品促销价 -->
												<img id="goodsimg" src="/userphoto/${goods.goodsimage}" width="218px" height="218px"/><!-- 商品图片 -->											
												<p id="goodsname" class="title fl">${goods.goodsname}</p><!-- 商品名称 -->
												<p class="price fl">
													<b>¥</b>
													<strong id="goodsprice">${goods.goodsprice}</strong><!-- 商品价格 -->
												</p>
												<p class="number fl">
													销量<span id="goodsxiaoliang">${goods.goodssell}</span><!-- 商品销量 -->
												</p>
											</div>
										</li>
...
<!-- 									<li> -->
<!-- 										<div class="i-pic limit"> -->
<!-- 											<img src="res/static/one/images/imgsearch1.jpg" />											 -->
...
<!-- 									</li> -->
							...
							<div class="search-side">
								...
								<li>
									<div class="i-pic check">
										<img src="res/static/one/images/cp2.jpg" />
								...
								<li>
									<div class="i-pic check">
										<img src="res/static/one/images/cp.jpg" />
							...
							<script type="text/javascript">
								...
								function nextpage(){/*****当点击下一页的按钮时触发*****/
									//获取地址栏的title，确定用户是否输入
									var title=getUrlSearch('title');
									if(title!='no'){
										//用户输入了搜索内容
										//确定用户是否是排序后的分页显示
										var dr=getUrlSearch('dr');
										//alert('dr='+dr);
										if(dr=='sales'){
											//alert('按销量排序分页')
											//按销量排序后分页(下一页)
											...
											var current=getUrlSearch('current');
											current++;//页数++
											location.href="goods?method=inputSortSalesPaging&current="+current;
											return;
										}
										if(dr=='price'){
											//按价格排序后分页
											current=getUrlSearch('current');//获取当前页数
											current++;//查询下一页的商品
											location.href="goods?method=inputSortpricepaging&current="+current;
											return;
										}
										
										if(title!="no"){
											//获取用户输入的内容
											title=getUrlSearch('title');
											//获取当前页数
											current=getUrlSearch('current')
											current++;//查询下一页的商品
											//请求后台 查询指定页的商品(默认排序)
											location.href="goods?method=inputSortPaging&title="+title+"&current="+current;
										}else{
											//用户没有输入内容
											//默认查出来的所有商品分页
											location.href="goods?method=paging&current="+current;
										}
										return;
									}
									current++;//页数+1
									...
									if(dr=='sales'){
										//alert('按销量排序分页')
										//按销量排序后分页
										location.href="goods?method=salespaging&current="+current;
										return;
									}
									if(dr=='price'){
										//按价格排序后分页
										location.href="goods?method=pricepaging&current="+current;
										return;
									}
									//所有商品分页
									location.href="goods?method=paging&current="+current;
								...
								function prevpage(){/*******当点击上一页按钮时触发*************/
									...
									//用户没输入
									currentpage=getUrlSearch('current');//获取页数
									currentpage--;//页数-1
									...
									//alert('dr='+dr);
									if(dr=='sales'){
										currentpage=getUrlSearch('current');
										currentpage--;//页数-1
										//按销量排序后分页
										location.href="goods?method=salespagingup&current="+currentpage;
										return;
									}
									if(dr=='price'){
										//按价格排序后分页
										location.href="goods?method=pricepaging&current="+currentpage;
										return;
									}
									location.href="goods?method=paging&current="+currentpage;
								}
							</script>
