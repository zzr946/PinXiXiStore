	...
	<nav class="navbar navbar-default">
		<div class="container-fluid">
			<div class="navbar-header">
				<a href="" class="navbar-brand">商家封禁界面</a>
			</div>
...
<!-- 		数据库区 -->
		<div class="data-container">
			<table class="table table-striped table-bordered" id="ta">
				...
				<c:forEach var="repo" items="${report}" varStatus="d">
					<tr>
						<td>${repo.mid}</td>
						<td>${repo.cause}</td>
						<td>${repo.content}</td>
						<td>${repo.reporttime}</td>
...
<!-- 							<button class="btn btn-info btn-sm btnUpdate" data-toggle="modal"  data-target="#dailyEdit"><span class="glyphicon glyphicon-edit"></span>修改</button> -->
							<button  class="btn btn-danger btn-sm btnDel" onclick="nimabi(this)"><span class="glyphicon glyphicon-remove"></span>封禁</button>
						</td>
					</tr>
				</c:forEach>
			</table>
	...
	function nimabi(btn){
		...
		location.href="../../admin1/admin?method=Admin_banTheMerchant&mid="+id1;
		});
	}
	
	$(function(){
		$('.btnInfo').click(function(){
			var todatask = $(this).parent().parent().children(':nth-child(1)').text();
// 			console.info(todatask);
		$('#d1').text(todatask);
		
		var completeinfo = $(this).parent().parent().children(':nth-child(2)').text();
//			console.info(todatask);
	$('#d2').text(completeinfo);
	var nextplan = $(this).parent().parent().children(':nth-child(3)').text();
//		console.info(todatask);
$('#d3').text(nextplan);
